...
// Opus 4.7 pricing (per token). Verify against current Anthropic pricing.
...
export async function POST(req: Request) {
  ...
  const systemMessages = cache.buildCachedSystemMessages(stable, dynamic);
    ...
    execute: async ({ writer }) => {
      const result = streamText({
        ...
        onFinish: ({ usage, steps, totalUsage }) => {
          ...
          console.log(`\n[Route] ══ Complete ══`);
          console.log(
            `[Route] ${toolSummary || 'no tools'} · ~${totalToolTokens} tok to agent · ${steps.length} steps`,
          );
          console.log(
            `[Route] Tokens — in: ${totalUsage.inputTokens} · out: ${totalUsage.outputTokens} · context: ${usage.inputTokens} · ${((Date.now() - requestStart) / 1000).toFixed(1)}s`,
          );
          ...
          console.log(
            `[Route] Cache — read: ${cacheRead} · write: ${cacheWrite} · uncached: ${noCache} · hit: ${hitRate}%`,
          );
          console.log(
            `[Route] Cost  — in: $${(costRead + costWrite + costNoCache).toFixed(4)} · out: $${costOutput.toFixed(4)} · total: $${costTotal.toFixed(4)} · saved: $${savings.toFixed(4)}`,
          );
            ...
            if (cm?.appliedEdits?.length) {
              for (const edit of cm.appliedEdits) {
                if (edit.type === 'compact_20260112') {
                  console.log(
                    `[Route] ⚠ Compaction triggered — conversation was summarized`,
                  );
                } else if (edit.type === 'clear_tool_uses_20250919') {
                  console.log(
                    `[Route] Context edit — cleared ${edit.clearedToolUses} tool uses (${edit.clearedInputTokens} tokens)`,
                  );
                } else if (edit.type === 'clear_thinking_20251015') {
                  console.log(
                    `[Route] Context edit — cleared ${edit.clearedThinkingTurns} thinking turns (${edit.clearedInputTokens} tokens)`,
                  );
                }
              }
            }
