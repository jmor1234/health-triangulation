...
export const readTool = tool({
  ...
  inputSchema: z.object({
    ...
    query: z
      .string()
      .describe('What to focus on. Excerpts are selected by relevance to this query.'),
  }),
  execute: async ({ url, query }) => {
    ...
    console.log(
      `[Read]   ${domain} → "${query.substring(0, 60)}" · ${totalChars} chars · ${Date.now() - start}ms · ~${resultTokens} tok`,
    );
